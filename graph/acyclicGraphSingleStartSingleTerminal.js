...
var connectPoints = function(o_points,r){
    ...
    var unaddressed = {};
    unaddressed[pointmap[0]] = 1;
    for(var i = 1; i < pointmap.length; i++){
        //for every past points
        var r1 = r;
        var f1 = true;
        while(f1 && r1 < fieldWidth){
            for(var j = 0; j < i; j++){
                ...
                if(dist12 < r1){
                    ...
                    if(unaddressed[pointmap[j]])delete unaddressed[pointmap[j]];
                    f1 = false;
                }
            }
            r1*=1.2;
            unaddressed[pointmap[i]] = 1;
        }
    }
    for(var key in unaddressed){
        var lastPoint = pointmap[pointmap.length-1];
        graph[lastPoint].push([key,dist(points[key],points[lastPoint])]);
    }
    return graph;
};
