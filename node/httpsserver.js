...
const port = 443;
const HomeDir = "/home/yutaro/minesite/home";
...
var returnPage = function(requestInfo){
    //security
    if(!checkPath(requestInfo.globalPath))return false;
    fs.exists(requestInfo.globalPath, function(exists){
      if (exists) {
        ...
        if(stats.isDirectory()){
            requestInfo.globalPath = path.join(requestInfo.globalPath,"/index.html");
            requestInfo.extname = ".html";
...
//insert code below for security
//if(!checkPath(requestInfo.globalPath))return false;
var checkPath = function(p){
    if(path.join(p).indexOf(HomeDir) !== 0){
        return false;
    }
    return true;
};
